...
void main() {
    ...
    // pattern 25
    // vec2 gridUv = vec2(floor(vUv.x * 10.0), floor((vUv.y + vUv.x * 0.5) * 10.0));
    // float strength = random(gridUv);
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
    ...
    // angle /= PI * 2.0;
    // angle += 0.5;
    // float sinus = sin(angle * 100.0);
    // float radius = 0.25 + sinus * 0.02;
    // float strength = 1.0 - step(0.01, abs(distance(vUv, vec2(0.5)) - radius));
    // gl_FragColor = vec4(strength, strength, strength, 1.0);

    // pattern 46
    // float strength = cnoise((vUv - 0.5) * 10.0);
    // gl_FragColor = vec4(strength, strength, strength, 1.0);

    // pattern 47
    // float strength = step(0.0, cnoise((vUv - 0.5) * 10.0));
    // gl_FragColor = vec4(strength, strength, strength, 1.0);

    // pattern 48
    // float strength = 1.0 - abs(cnoise((vUv - 0.5) * 10.0));
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
    ...
    // clamped the strength to 0.0 - 1.0
    strength = clamp(strength, 0.0, 1.0);
    // float minimum = min(-1.0, 1.0);
    ...
    // float wave = sin(angle * 10.0 + radius * 50.0);
    // float strength = 1.0 - step(0.01, abs(wave - 0.0));
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
}