...
gltfLoader.load(
    ...
    (gltf) => {
        ...
        const randomMorphLoop = () => {
            let nextIndex = Math.floor(Math.random() * particles.positions.length);
            particles.morph(nextIndex);
            setTimeout(randomMorphLoop, 3500); // wait morph duration + buffer
        };
        ...
        // gui.add(particles.material.uniforms.uSize, 'value').min(0).max(2).step(0.01).name('uSize')  
    }
)
